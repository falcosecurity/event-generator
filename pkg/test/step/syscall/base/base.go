...
func New(name string, rawArgs map[string]string, fieldBindings []*step.FieldBinding, argsContainer,
...
func setArgFieldValues(argFieldContainer reflect.Value, rawArgs map[string]string) (map[string]struct{}, error) {
	...
	for rawArg, rawArgValue := range rawArgs {
		...
		if err != nil {
			return nil, fmt.Errorf("error getting %q argument field info: %v", rawArg, err)
		}

		if err := setArgFieldValue(argField, rawArgValue); err != nil {
			return nil, fmt.Errorf("error setting %q argument field value to %q: %v", rawArg, rawArgValue, err)
		}
...
func setArgFieldValue(argField *field.Field, value string) error {
	...
	case field.TypeSocketDomain:
		...
		if err != nil {
			return fmt.Errorf("cannot parse value as socket domain: %v", err)
		}
		argFieldValue.SetInt(int64(socketDomain))
	case field.TypeSocketType:
		socketType, err := parseSocketType(value)
		if err != nil {
			return fmt.Errorf("cannot parse value as socket type: %v", err)
		}
