...
func (l *Loader) Load(r io.Reader) (*Description, error) {
	...
	// Instantiate test templates, if any.
	rawDesc, sourceTestPositions, generatingTestCases, err := instantiateTestTemplates(rawDesc)
	if err != nil {
		return nil, fmt.Errorf("error instantiating test templates: %w", err)
	}
	...
	// Assign to each test the information regarding its originating test case (if any) and its original source
	// position.
	for testIndex := range desc.Tests {
		testDesc := &desc.Tests[testIndex]
		testDesc.OriginatingTestCase = generatingTestCases[testIndex]
		testDesc.SourceIndex = sourceTestPositions[testIndex]
	}
...
// TestCase groups test case values by value key.
